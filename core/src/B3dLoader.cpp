            ...
            const glm::quat &rot)
{
    ...
    b.name = name;
    b.parent = parent;
    b.localPose = makeTRS(pos, rot, scale);
                    ...
                    int parentBone)
{
    ...
    const glm::vec3 sc(s.readF32(), s.readF32(), s.readF32());
    ...
    const int myBone = addBone(mesh, boneAnim, nodeName, parentBone, pos, sc, rot);
    ...
    while (stack.remaining(s) > 0)
    {
        ...
        if (tag == "MESH")
        {
            ...
            const int brushIdUnused = s.readI32();
            (void)brushIdUnused;
            ...
            while (stack.remaining(s) > 0)
            {
                ...
                if (mtag == "VRTS")
                {
                    ...
                    for (int i = 0; i < count; ++i)
                    {
                        ...
                        for (int t = 0; t < numUV; ++t)
                        {
                            ...
                            for (int k = 2; k < uvSize; ++k) s.readF32();
                            if (t == 0) v.uv = glm::vec2(u, vv);
                        }
                ...
                else if (mtag == "TRIS")
                {
                    ...
                    PendingSurface ps;
                    ps.brushId = brushId;
                    ps.indexStart = static_cast<uint32_t>(mesh.buffer.indices.size());
                    ...
                    ps.indexCount = static_cast<uint32_t>(mesh.buffer.indices.size()) - ps.indexStart;
                    if (ps.indexCount > 0)
        ...
        {
            const int flags = s.readI32();
            BoneAnimData &ba = boneAnim[static_cast<size_t>(myBone)];
            while (stack.remaining(s) > 0)
            {
                const float t = static_cast<float>(s.readI32());
                if (flags & 1)
                {
                    ba.pos.push_back({t, glm::vec3(s.readF32(), s.readF32(), s.readF32())});
                }
                if (flags & 2)
                {
                    ba.scale.push_back({t, glm::vec3(s.readF32(), s.readF32(), s.readF32())});
                }
                if (flags & 4)
                {
...
bool B3dLoader::load(const std::string &b3dPath,
                     const std::string &textureDir,
                     AnimatedMesh *outMesh,
                     std::vector<Animation *> *outAnimations) const
{
    if (!outMesh)
        return false;

    outMesh->name = PathStem(b3dPath);
    ...
    BinaryStream s(b3dPath, "rb");
    if (!s.isOpen())
    {
        LogError("[B3dLoader] Failed to open: %s", b3dPath.c_str());
        return false;
    }
    ...
    if (rootTag != "BB3D")
    {
        LogError("[B3dLoader] Invalid root tag '%s' in %s", rootTag.c_str(), b3dPath.c_str());
        return false;
    }

    ChunkStack stack;
    stack.push(s, static_cast<uint32_t>(s.readI32()));
    const int version = s.readI32();
    (void)version;
    ...
    while (stack.remaining(s) > 0)
    {
        ...
        if (tag == "TEXS")
        {
            while (stack.remaining(s) > 0)
            {
                ...
                s.readI32(); // flags
                s.readI32(); // blend
                s.readF32(); s.readF32(); // pos
                s.readF32(); s.readF32(); // scale
                s.readF32(); // rot
        ...
        else if (tag == "BRUS")
        {
            ...
            while (stack.remaining(s) > 0)
            {
                BrushEntry be;
                readCString(s); // name
                s.readF32(); s.readF32(); s.readF32(); s.readF32(); // rgba
                s.readF32(); // shininess
                s.readI32(); // blend
                s.readI32(); // fx
    ...
    {
        LogError("[B3dLoader] No geometry in %s", b3dPath.c_str());
        return false;
    }

    const int maxShaderBones = 100;
    if (static_cast<int>(outMesh->bones.size()) > maxShaderBones)
    {
        LogWarning("[B3dLoader] %s has %zu bones, shader supports %d (IDs will be clamped)",
                    b3dPath.c_str(), outMesh->bones.size(), maxShaderBones);
    }

    // Build bind globals and offsets
    if (!outMesh->bones.empty())
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            const int p = outMesh->bones[i].parent;
            if (p >= 0 && p < n)
                global[i] = global[p] * outMesh->bones[i].localPose;
            else
    ...
    // Apply skin weights to vertices
    for (size_t i = 0; i < outMesh->buffer.vertices.size(); ++i)
    {
        auto &v = outMesh->buffer.vertices[i];
    ...
    // Load texture slots referenced by TEXS
    const std::string baseDir = textureDir.empty() ? PathDirectory(b3dPath) : textureDir;
    ...
    for (size_t i = 0; i < surfaces.size(); ++i)
    {
        ...
        const std::string matName = outMesh->name + "::surf_" + std::to_string(i);
        Material *mat = MaterialManager::instance().create(matName);
    ...
    if (outMesh->bones.empty())
    {
        ...
        for (auto &v : outMesh->buffer.vertices)
        {
            v.boneIds = glm::ivec4(0, 0, 0, 0);
            v.boneWeights = glm::vec4(1.0f, 0.0f, 0.0f, 0.0f);
        }
    ...
    if (outAnimations && !outMesh->bones.empty())
    {
        auto *anim = new Animation();
        anim->name = "default";
        anim->ticksPerSecond = (animFps > 0.0f) ? animFps : 25.0f;
        anim->duration = (animFrames > 1) ? static_cast<float>(animFrames - 1) : 1.0f;
        ...
        for (size_t i = 0; i < outMesh->bones.size(); ++i)
        {
            ...
            auto &src = boneAnim[i];
            ch.boneName = outMesh->bones[i].name;
            ...
            {
                ch.posKeys.push_back({0.0f, src.bindPos});
            }
            ...
            {
                ch.rotKeys.push_back({0.0f, src.bindRot});
            }
            ...
            if (!src.scale.empty())
            {
                ...
                ch.scaleKeys = src.scale;
            }
            else
            {
                ch.scaleKeys.push_back({0.0f, src.bindScale});
            }
        }
    ...
    LogInfo("[B3dLoader] '%s' loaded: verts=%zu tris=%zu bones=%zu surfs=%zu anims=%zu",
            b3dPath.c_str(),
