...
bool Animation::load(const std::string &path)
{
    ...
    if (!f.is_open())
    {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "[Animation] Cannot open: %s", path.c_str());
        return false;
    }

    AnimationChannel *cur = nullptr;
    std::string line;
    ...
    while (std::getline(f, line))
    {
        ...
        {
            PosKey k;
            ss >> k.time >> k.value.x >> k.value.y >> k.value.z;
            cur->posKeys.push_back(k);
        }
        else if (cur && cmd == "rot")
        {
            RotKey k;
            ss >> k.time >> k.value.x >> k.value.y >> k.value.z >> k.value.w;
            k.value = glm::normalize(k.value);
            cur->rotKeys.push_back(k);
        }
        else if (cur && cmd == "scale")
        {
            ScaleKey k;
            ss >> k.time >> k.value.x >> k.value.y >> k.value.z;
            cur->scaleKeys.push_back(k);
        }
    }

    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                "[Animation] Loaded '%s' — duration=%.1f tps=%.1f channels=%zu",
                name.c_str(), duration, ticksPerSecond, channels.size());
    return true;
}