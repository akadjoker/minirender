...
public:
    ...
    // ── Bone transforms (mesh-local, sem inverse bind pose) ─────────
    // Usados para BoneSocket / attachments — válidos após update()
...
private:
    ...
    std::vector<int>       boneOrder_;         // BFS traversal order
    std::vector<glm::mat4> globalMatrices_;    // mesh-local bone transforms (no offset)
