...
{
    Frustum f;
    // normal=(0,1,0), d=1e30 → distance(p) = p.y + 1e30 ≥ 0 for any sane coord
    for (auto &p : f.planes)
    {
...
// Returns t > 0 on hit, -1 on miss or back-face cull.
float Triangle::intersect_ray(const glm::vec3 &orig, const glm::vec3 &dir) const
{
    ...
    if (std::fabs(a) < EPS) return -1.f; // parallel or degenerate
