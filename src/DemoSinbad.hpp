...
public:
    ...
    bool init() override
    {
        ...
        // Upper layer starts dormant; layer 0 controls full body
    ...
    void update(float dt) override
    {
        ...
        if (in.isKeyDown(SDL_SCANCODE_1)) { lowerLayer->crossFade("IdleBase", 0.3f); currentAnim = "IdleBase"; lowerLayer->setSpeed(1.0f); }
        if (in.isKeyDown(SDL_SCANCODE_2)) { lowerLayer->crossFade("RunBase",  0.2f); currentAnim = "RunBase";  lowerLayer->setSpeed(1.0f); }
        if (in.isKeyDown(SDL_SCANCODE_3)) { lowerLayer->crossFade("Dance",    0.3f); currentAnim = "Dance";    lowerLayer->setSpeed(1.0f); }

        // ── Ataques (layer 1 — upper body apenas) ────────────
        // As pernas continuam a correr/idle enquanto os braços atacam!
        if (upperLayer)
        {
            if (in.isKeyDown(SDL_SCANCODE_4)) { upperLayer->setSpeed(0.2f);  upperLayer->play("DrawSwords",      PlayMode::Once); }
            if (in.isKeyDown(SDL_SCANCODE_5)) { upperLayer->setSpeed(0.5f); upperLayer->play("SliceVertical",   PlayMode::Once); }
            if (in.isKeyDown(SDL_SCANCODE_6)) { upperLayer->setSpeed(0.8f);  upperLayer->play("SliceHorizontal", PlayMode::Once); }
        }

        // ── Salto — full body (layer 0) ───────────────────────
        if (in.isKeyDown(SDL_SCANCODE_SPACE))
        {
            lowerLayer->setSpeed(1.0f);
            lowerLayer->playOneShot("JumpStart", "JumpLoop");
        }
        if (in.isKeyDown(SDL_SCANCODE_R) && lowerLayer->isPlaying("JumpLoop"))
            lowerLayer->playOneShot("JumpEnd", currentAnim);

        // ── T — cicla mapeamento de materiais para debug ──────
        if (in.isKeyDown(SDL_SCANCODE_T))
        {
