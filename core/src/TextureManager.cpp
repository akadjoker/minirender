...
void applyPixmapComponentSwizzle(GLuint textureId, int components)
{
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_A, components == 2 ? GL_GREEN : GL_ONE);
...
// ─── private helper: raw GL upload ──────────────────────────────────────────
Texture *TextureManager::uploadMemory(const std::string &name,
                                      ...
                                      const TextureLoadOptions &opts)
{
    ...
    {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "[TextureManager] uploadMemory invalid args for '%s'",
                     name.c_str());
        return nullptr;
    }
    ...
    {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "[TextureManager] uploadMemory unsupported PixelType (%u) for '%s'",
    ...
    {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "[TextureManager] uploadMemory small buffer for '%s' (%zu < %zu)",
...
// ─── public wrapper — uses defaultOpts_ ──────────────────────────────────────
                                          ...
                                          std::size_t sizeBytes)
{
    ...
    return uploadMemory(name, width, height, pixelType, data, sizeBytes, defaultOpts_);
}
                                   ...
                                   uint8_t r, uint8_t g, uint8_t b, uint8_t a)
{
    ...
    return uploadMemory(name, 1, 1, PixelType::RGBA, pixels, sizeof(pixels), opts);
}

Texture *TextureManager::createFromPixmap(const std::string &name, const Pixmap &pixmap)
{
    const TextureLoadOptions &opts = defaultOpts_;
    if (Texture *existing = get(name))
    {
    ...
    if (pixmap.components <= 0 || pixmap.width <= 0 || pixmap.height <= 0 || !pixmap.pixels || pixmap.components > 4)
    {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "[TextureManager] createFromPixmap invalid Pixmap for '%s'", name.c_str());
        return nullptr;
    }
    ...
    default:
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                     "[TextureManager] createFromPixmap unsupported component count (%d) for '%s'",
                     pixmap.components, name.c_str());
        return nullptr;
    }

    const std::size_t expectedSize = static_cast<std::size_t>(pixmap.width) *
                                     static_cast<std::size_t>(pixmap.height) *
                                     static_cast<std::size_t>(pixmap.components);
    Texture *texture = uploadMemory(name, pixmap.width, pixmap.height,
                                    pixelType, pixmap.pixels, expectedSize, opts);
    if (texture)
...
Texture *TextureManager::getPattern()
{
    ...
    opts.filterMin  = GL_NEAREST;
    opts.filterMag  = GL_NEAREST;
    return uploadMemory("__pattern", 64, 64, PixelType::RGBA, pixels.data(), pixels.size(), opts);
}
