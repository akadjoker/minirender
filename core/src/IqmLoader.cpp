...
#pragma pack(pop)
                         ...
                         std::vector<glm::vec3> &outScale)
    {
        ...
        for (size_t i = 0; i < poseCount; ++i)
        {
            ...
            glm::quat q(ch[6], ch[3], ch[4], ch[5]); // iqm stores xyzw
...
bool IqmLoader::parseSkinFile(const std::string &path,
                              std::unordered_map<std::string, std::string> &outMap)
{
    ...
    while (std::getline(in, line))
    {
        ...
        const std::string meshName = trim(line.substr(0, comma));
        const std::string texName = trim(line.substr(comma + 1));
...
bool IqmLoader::parseFrameGroups(const std::string &path,
                                 std::vector<FrameGroupClip> &outClips)
{
    ...
    while (std::getline(in, line))
    {
        ...
        {
            name = trim(line.substr(commentPos + 2));
...
bool IqmLoader::load(const std::string &iqmPath,
                     ...
                     std::vector<Animation *> *outAnimations) const
{
    ...
    if (!s.isOpen())
    {
        LogError("[IqmLoader] Failed to open: %s", iqmPath.c_str());
        return false;
    }
    ...
    {
        LogError("[IqmLoader] File too small: %s", iqmPath.c_str());
        return false;
    }
    ...
    {
        LogError("[IqmLoader] Invalid IQM header/version in %s", iqmPath.c_str());
        return false;
    }

    if (hdr.num_vertexes == 0 || hdr.num_triangles == 0 || hdr.num_meshes == 0)
    {
        LogError("[IqmLoader] Empty IQM geometry in %s", iqmPath.c_str());
        return false;
    }
    ...
    if (hdr.num_text > 0)
    {
        ...
        {
            LogError("[IqmLoader] Invalid IQM text table in %s", iqmPath.c_str());
            return false;
        }
    ...
    {
        LogError("[IqmLoader] Corrupt geometry chunks in %s", iqmPath.c_str());
        return false;
    }

    if (hdr.num_joints > 0 &&
        !readArrayAt(s, fileSize, hdr.ofs_joints, hdr.num_joints, joints))
    {
        LogError("[IqmLoader] Corrupt joints chunk in %s", iqmPath.c_str());
        return false;
    }

    if (hdr.num_poses > 0 &&
        !readArrayAt(s, fileSize, hdr.ofs_poses, hdr.num_poses, poses))
    {
        LogError("[IqmLoader] Corrupt poses chunk in %s", iqmPath.c_str());
        return false;
    }

    if (hdr.num_anims > 0 &&
        !readArrayAt(s, fileSize, hdr.ofs_anims, hdr.num_anims, iqmAnims))
    {
        LogError("[IqmLoader] Corrupt anim chunk in %s", iqmPath.c_str());
        return false;
    }
    ...
    if (frameCount > 0)
    {
        ...
        {
            LogError("[IqmLoader] Corrupt frame data in %s", iqmPath.c_str());
            return false;
        }
    ...
    if (!joints.empty())
    {
        ...
        for (int i = 0; i < jointCount; ++i)
        {
            ...
            const glm::quat q(j.rotate[3], j.rotate[0], j.rotate[1], j.rotate[2]); // w, x, y, z
            const glm::vec3 sc(j.scale[0], j.scale[1], j.scale[2]);

            localBind[i] = makeTRS(t, glm::normalize(q), sc);

            Bone b;
            b.name = iqmTextAt(text, j.name);
            b.parent = j.parent;
            b.localPose = localBind[i];
            outMesh->bones[i] = b;
        }

        for (int i = 0; i < jointCount; ++i)
        {
            const glm::mat4 g = evalGlobalBindRecursive(i, joints, localBind, globalBind, state);
            outMesh->bones[i].offset = glm::inverse(g);
        }
    }
    ...
    if (clampBoneIds)
    {
        LogWarning("[IqmLoader] %s has %zu bones, shader supports %d; clamping bone IDs",
                   iqmPath.c_str(),
                   outMesh->bones.size(),
                   maxShaderBones);
    }
    ...
    for (const IqmVertexArray &va : varrays)
    {
        ...
        {
            for (size_t i = 0; i < vertexCount; ++i)
                uvs[i] = glm::vec2(s.readF32(),1.0f - s.readF32());
        }
    ...
    for (size_t i = 0; i < vertexCount; ++i)
    {
        ...
        float sum = boneWeights[i].x + boneWeights[i].y + boneWeights[i].z + boneWeights[i].w;
        if (sum > 1e-8f)
    ...
    // Optional skin remap from IQM companion files
    ...
    for (size_t i = 0; i < skinCandidates.size(); ++i)
    {
        if (FileExists(skinCandidates[i]) && parseSkinFile(skinCandidates[i], skinMap))
            break;
    }
    ...
    for (size_t i = 0; i < meshes.size(); ++i)
    {
        ...
        const std::string matId =
            outMesh->name + "::" + (meshName.empty() ? ("mesh_" + std::to_string(i)) : meshName);
        Material *mat = MaterialManager::instance().create(matId);
        Texture *albedo = nullptr;
        if (!texPath.empty())
            albedo = TextureManager::instance().load(matId + "::albedo", texPath);
    ...
    // Build animations, preferring explicit framegroups when present.
    if (outAnimations)
    {
    ...
    LogInfo("[IqmLoader] '%s' loaded: verts=%zu tris=%zu bones=%zu surfs=%zu anims=%zu",
            iqmPath.c_str(),
